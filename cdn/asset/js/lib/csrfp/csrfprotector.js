...
var CSRFP = {
  ...
  _init: function () {
    ...
    try {
      CSRFP.checkForUrls = JSON.parse(
        document.getElementById(CSRFP_FIELD_URLS).value
      );
    } catch (err) {
      console.error(err);
      console.error(
        "[ERROR] [CSRF Protector] unable to parse blacklisted url fields."
      );
    }

    //convert these rules received from php lib to regex objects
    for (var i = 0; i < CSRFP.checkForUrls.length; i++) {
      CSRFP.checkForUrls[i] = CSRFP.checkForUrls[i]
        .replace(/\*/g, "(.*)")
        .replace(/\//g, "\\/");
...
function csrfprotector_init() {
   ...
   * todo - check for method
   */
  HTMLFormElement.prototype.addEventListener_ =
  ...
  if (typeof HTMLFormElement.prototype.attachEvent !== "undefined") {
    HTMLFormElement.prototype.attachEvent_ =
  ...
  function new_send(data) {
    try {
      if (
        ...
        !CSRFP._isValidGetRequest(this.url)
      ) {
